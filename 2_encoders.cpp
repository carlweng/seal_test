...
void test_integer_encoder()
{
    ...
    //BlakePRNGFactory prngf(rseed);
    //auto prng = prngf.create();
    //std::cout << "\t generate random number: " << prng->generate() << std::endl;

    print_example_banner("Test: Encoders / Integer Encoder");
    ...
    evaluator.negate(encrypted2, encrypted_result);
    evaluator.add_inplace(encrypted_result, encrypted1);
    evaluator.multiply_inplace(encrypted_result, encrypted1);
